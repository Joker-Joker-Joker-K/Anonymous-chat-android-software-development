...
//        列表的显示会更新，显示最新的数据。
    ...
    class T_IViewHolder extends RecyclerView.ViewHolder{
        private TextView my_tv,my_tv2;
        public T_IViewHolder(View itemView) {
            super(itemView);
            my_tv = (TextView) itemView.findViewById(R.id.all_my_msg_tv);
            my_tv2 = (TextView) itemView.findViewById(R.id.all_my_msg_tv2);
        }
    }

    class T_HeViewHolder extends RecyclerView.ViewHolder{
        private TextView his_tv,his_tv2;
        public T_HeViewHolder(View itemView) {
            super(itemView);
            his_tv = (TextView) itemView.findViewById(R.id.all_his_msg_tv);
            his_tv2 = (TextView) itemView.findViewById(R.id.all_his_msg_tv2);
        }
    }
...
//        它表示每个列表项的视图容器，用于保存列表项中的视图控件引用。
        ...
        if (viewType==T_I){
            View view = LayoutInflater.from(context).inflate(R.layout.all_mymsg_layout,parent,false);
...
//            通过调用 inflate() 方法，将布局文件转换为一个视图对象，并将其存储在 view 变量中供后续使用。
            ...
            View view = LayoutInflater.from(context).inflate(R.layout.all_hismsg_layout,parent,false);
//            View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.fruit_item,null);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (itemViewType==T_I) {
            ...
            myViewHolder.my_tv2.setText(Msg.get(position).getName());
        }
        else{
            T_HeViewHolder HisViewHolder = (T_HeViewHolder) holder;
            HisViewHolder.his_tv.setText(Msg.get(position).getMsg());
            HisViewHolder.his_tv2.setText(Msg.get(position).getName());
        }
    }
